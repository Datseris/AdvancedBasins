...
# basins, attractors = basins_of_attraction(mapper, reduced_grid)
...
for i=1:length(attractors) scatter!(ax, Matrix(attractors[i])[:,1], Matrix(attractors[i])[:,2], color=:black, markersize=10) end
    ...
    # mesh!(ax, Vector(xg), Vector(yg), Vector(zg), basins)
    # surface!(ax, xg, yg, zg, basins)
